...
<div id="modal_cetak" class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/50 backdrop-blur-sm hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-200 transform transition-all">
        ...
        <form id="form_cetak_detail" class="space-y-4" onsubmit="handlePrintSubmit(event)" novalidate>
            ...
            <div id="validation_msg" class="hidden text-sm font-semibold text-rose-600 bg-rose-50 border border-rose-200 rounded-lg px-3 py-2"></div>
...
<script>
    ...
    function handlePrintSubmit(e) {
        ...
        if (letterType === 'panggilan_ortu') {
            ...
            if (!tanggal) { showValidationMsg('Tanggal kehadiran wajib diisi!'); return; }
            if (!jam)     { showValidationMsg('Jam kehadiran wajib diisi!'); return; }
        }

        if (letterType === 'sp') {
            const spId = document.getElementById('sp_select').value;
            if (!spId) { showValidationMsg('Pilih Surat Peringatan terlebih dahulu!'); return; }
        }
